...
module "glue" {
  ...
  number_of_workers  = 10     # ← Change from 5
  max_retries        = 2      # ← Change from 1
  job_timeout        = 120    # ← Change from 60
  log_retention_days = 90     # ← Change from 30
...
module "athena" {
  ...
  # Prod: 90-day query result retention (fast cleanup)
  query_results_retention_days = 90
